...
class Engine{
    ...
    createPlayer(socket, name){
        if(this.gameData.get('allied') + this.gameData.get('axis') < this.gameData.get('max')){
            const delta = this.gameData.get('max') - (this.gameData.get('allied') + this.gameData.get('axis')) - 1
			if(this.gameData.get('allied') <= this.gameData.get('axis')){
                ...
                socket.emit(Constants.SOCKET_MSG, `Allied tank. ${(delta != 0) ? `Please wait for ${delta} more players to join.` : ``}`)
			}
			else{
				...
				socket.emit(Constants.SOCKET_MSG, `Axis tank. ${(delta != 0) ? `Please wait for ${delta} more players to join.` : ``}`)
            }
		}
		else{
